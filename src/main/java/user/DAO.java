...
public class DAO {
	...
	protected List<String> getAllUserId() throws SQLException {
	...
	protected void add(User user) throws SQLException {
		...
		try {
			String query = "INSERT INTO " + "USER(USER_EMAIL, USER_ID, USER_PASSWORD, "
					+ "USER_NAME, USER_ROLE, SEND_EMAIL) " + "VALUES(?,?,?,?,?,?)";
			PreparedStatement ps = conn.prepareStatement(query);
	...
	protected boolean isLoginCorrect(String tryEmail, String hashedTryPassword) throws SQLException {
	...
	protected User getAttributes(HttpSession loggedIn) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
				...
				user.setSendEmail(rs.getString("SEND_EMAIL"));
			}
			return user;
		} catch (SQLException e) {
			throw new SQLException(e);
